...
name: watch
version: "0.2.0"
description: Watch a video (URL or local path). Downloads with yt-dlp, extracts auto-scaled frames with ffmpeg, pulls the transcript from captions (or ASR fallback), and hands the result to Claude so it can answer questions about what's in the video.
...
You don't have a video input; this skill gives you one. A Python script gets captions first, optionally downloads the video, extracts frames as JPEGs (scene-aware, or fast keyframes at `efficient` detail), gets a timestamped transcript (native captions first, then ASR API fallback), and prints frame paths. You then `Read` each frame path to see the images and combine them with the transcript to answer the user.
...
| `3` | Genuine first run with no ASR API key | Run installer to scaffold `.env`, then encourage a key (the user may decline — proceed with `--no-whisper`) |
...
**If an API key is still missing after install:** ask whether the user has a Groq, OpenAI, or Aliyun DashScope key. Then write it into `~/.config/watch/.env` — set `GROQ_API_KEY=...`, `OPENAI_API_KEY=...`, or `DASHSCOPE_API_KEY=...`. If they don't want to set up ASR, proceed with `--no-whisper` and tell them videos without native captions will come back frames-only.
...
- `--whisper groq|openai|aliyun` — force a specific ASR backend (default: Groq, OpenAI, then Aliyun)
- `--no-whisper` — disable the ASR fallback entirely (frames-only if no captions)
...
- **Transcript** — what's said at each timestamp. The report's header shows the source (`captions` = yt-dlp pulled native subs; `asr (groq)`, `asr (openai)`, or `asr (aliyun)` = transcribed by API).
...
At `transcript` detail, captions are enough to return a report without downloading video. If captions are missing, the script downloads audio only and tries ASR. If no transcript can be produced, it reports the limitation clearly; re-run with `--detail balanced` for frames.
...
2. **ASR API fallback.** If no captions came back (or the source is a local file), the script extracts audio (`ffmpeg -vn -ac 1 -ar 16000 -b:a 64k`, ~0.5 MB/min) and uploads it to whichever ASR API has a key configured:
   - **Groq** — `whisper-large-v3`. Preferred default: cheaper, faster. Get a key at console.groq.com/keys.
   - **OpenAI** — `whisper-1`. Fallback. Get a key at platform.openai.com/api-keys.
   - **Aliyun DashScope/Bailian** — non-realtime Paraformer file ASR. Default model order: `paraformer-v2,paraformer-v1`; override with `DASHSCOPE_ASR_MODELS`.

Keys live in `~/.config/watch/.env`. The script tries Groq, OpenAI, then Aliyun by default; override with `--whisper aliyun` to force Aliyun. Use `--no-whisper` to skip the fallback entirely.
...
- **No transcript available** → captions missing AND (no ASR key OR ASR API failed). Script prints a hint pointing to setup. Proceed frames-only and tell the user.
- **Long video warning printed** → acknowledge it in your answer. Offer to re-run focused on a specific section via `--start`/`--end` rather than a sparse full-video scan.
- **Download fails** → yt-dlp's error goes to stderr. If it's a login-required or region-locked video, tell the user plainly; do not keep retrying.
- **ASR request fails** → the error is printed to stderr (likely: invalid key, model not enabled, quota, or rate limit). Audio over the Whisper 25 MB upload cap is split into chunks and transcribed automatically; Aliyun also has its own model fallback order. The report will say "none available" only if every chunk fails. You can retry with `--whisper aliyun`, `--whisper openai`, or `--whisper groq` to force one backend.
...
- Sends the extracted audio clip to Aliyun DashScope temporary OSS + Paraformer file ASR when `DASHSCOPE_API_KEY` is set, or when `--whisper aliyun` is forced
- Writes the downloaded video, frames, audio, and an intermediate transcript to a working directory under the system temp dir (or `--out-dir` if specified) so Claude can `Read` them
- Reads / creates `~/.config/watch/.env` (mode `0600`) to store ASR API key(s), optional Aliyun model settings, and a `SETUP_COMPLETE` marker. As a fallback, also reads `.env` in the current working directory

**What this skill does NOT do:**
- Does not upload the video itself to any API — only the extracted audio goes out, and only when native captions are missing AND ASR is not disabled with `--no-whisper`
- Does not access any platform account (no login, no session cookies, no posting) — yt-dlp only ever requests public data
- Does not share API keys between providers (Groq key only goes to `api.groq.com`, OpenAI key only goes to `api.openai.com`, Aliyun key only goes to DashScope/Bailian endpoints)
- Does not log, cache, or write API keys to stdout, stderr, or output files
- Does not persist anything outside the working directory and `~/.config/watch/.env` — clean up the working directory when you're done (Step 5)

**Bundled scripts:** `scripts/watch.py` (entry point), `scripts/download.py` (yt-dlp wrapper), `scripts/frames.py` (ffmpeg frame extraction), `scripts/transcribe.py` (caption selection), `scripts/whisper.py` (ASR orchestration), `scripts/aliyun_asr.py` (Aliyun Paraformer client), `scripts/setup.py` (preflight + installer)
