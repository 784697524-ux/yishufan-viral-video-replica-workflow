...
def main() -> int:
    ...
    ap.add_argument(
        "--whisper",
        choices=["groq", "openai", "aliyun"],
        default=None,
        help="Force a specific ASR backend. Default: Groq, OpenAI, then Aliyun.",
    )
    ap.add_argument(
        "--no-dedup",
        action="store_true",
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and api_key:
            try:
                ...
                print(f"[watch] ASR fallback failed: {exc}", file=sys.stderr)
        else:
            hint = (
                f"--whisper {args.whisper} was set but the matching ASR API key is missing"
                if args.whisper else
                "no subtitles and no ASR API key found"
            )
            setup_py = SCRIPT_DIR / "setup.py"
            print(
                f"[watch] {hint} — run `python3 {setup_py}` to enable the ASR fallback",
                file=sys.stderr,
            )
