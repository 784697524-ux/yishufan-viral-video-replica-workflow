#!/usr/bin/env python3
"""Transcribe a video via Groq/OpenAI Whisper or Aliyun Paraformer ASR.

Strategy: extract audio (mono 16kHz mp3, tiny payload), upload to whichever
ASR API has a key. Returns segments in the same shape as transcribe.parse_vtt
...
from aliyun_asr import transcribe_file as transcribe_aliyun_file
...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    """Return (backend, api_key). Prefers Groq, falls back to OpenAI and Aliyun.

    If `preferred` is set, only that backend's key is considered.
    """
    candidates = load_api_keys(preferred)
    return candidates[0] if candidates else (None, None)


def load_api_keys(preferred: str | None = None) -> list[tuple[str, str]]:
    """Return available ASR backends in fallback order."""
...
def _transcribe_file(backend: str, api_key: str, audio_path: Path) -> list[dict]:
    ...
    if backend == "groq":
        ...
        segments, _ = transcribe_aliyun_file(audio_path, api_key)
        return segments
    else:
...
def transcribe_video(
    ...
    """
    if api_key:
        candidates = [(backend or "groq", api_key)]
    else:
        candidates = load_api_keys(backend)

    if not candidates:
        setup_py = Path(__file__).resolve().parent / "setup.py"
        raise SystemExit(
            "No ASR API key available. Set GROQ_API_KEY, OPENAI_API_KEY, or DASHSCOPE_API_KEY "
            "in the environment or in ~/.config/watch/.env. "
            f"Run `python3 {setup_py}` to configure."
        )

    print(
        "[watch] extracting audio for ASR "
    ...
    def transcribe_one(path: Path) -> list[dict]:
        nonlocal used_backend
        errors: list[str] = []
        for candidate_backend, candidate_key in candidates:
            try:
                segments = _transcribe_file(candidate_backend, candidate_key, path)
                used_backend = candidate_backend
                return segments
            except SystemExit as exc:
                errors.append(f"{candidate_backend}: {exc}")
                print(
                    f"[watch] ASR backend {candidate_backend} failed — trying next ({exc})",
                    file=sys.stderr,
                )
        raise SystemExit("ASR failed on every backend: " + " | ".join(errors))

    if audio_bytes <= MAX_UPLOAD_BYTES:
        print(
            f"[watch] audio: {audio_bytes / 1024:.0f} kB — uploading to ASR backend…",
            file=sys.stderr,
        )
    ...
    backend_label = used_backend or candidates[0][0]
