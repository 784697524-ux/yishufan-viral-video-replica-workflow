...
#
# ASR transcription fallback — used only when yt-dlp cannot get captions
# (or when you point /watch at a local file with no subtitles).
#
# Groq is preferred for Whisper, OpenAI is the compatible Whisper fallback,
# and Aliyun DashScope/Bailian Paraformer is the non-realtime ASR fallback.
#
# Get a Groq key:  https://console.groq.com/keys
# Get an OpenAI key:  https://platform.openai.com/api-keys
# Get an Aliyun key: https://bailian.console.aliyun.com/
#
# Leave all blank to disable ASR — /watch will still work, but videos
...
# Optional Aliyun ASR settings. Defaults are usually correct.
# DASHSCOPE_ENDPOINT=https://dashscope.aliyuncs.com
# DASHSCOPE_ASR_MODELS=paraformer-v2,paraformer-v1
...
def _status() -> dict:
    ...
    `status` describes the *ideal* state (an ASR key is encouraged), so a
    ...
    setup (consciously opting out of ASR). A keyless user who completed
...
def cmd_check() -> int:
    ...
    finished setup (SETUP_COMPLETE=true) counts as ready — ASR is
    ...
    if not s["has_api_key"] and not s["setup_complete"]:
        parts.append("no ASR API key (GROQ_API_KEY, OPENAI_API_KEY, or DASHSCOPE_API_KEY)")
...
def cmd_install() -> int:
    ...
    if has_key:
        ...
        print(f"[setup] ready. asr backend: {backend}")
        if installed_deps:
            print("[setup] installed dependencies; /watch is fully set up.")
        return 0

    print("")
    print("[setup] one step left: add an ASR API key.")
    print("")
    ...
    print("    DASHSCOPE_API_KEY=...  (Aliyun Paraformer non-realtime fallback)")
    print("")
